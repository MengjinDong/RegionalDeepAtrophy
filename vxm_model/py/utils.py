...
def get_file_list_intra_subject(root_dir, groups, csv_out, stages):
    ...
    with open(csv_out, 'w') as filename:
        for group in groups:
            for stage in stages:
                ...
                subject_list = root_dir + "/" + group + "/subject_list" + stage + ".csv"
                    ...
                    with open(subject_list) as f:
                        for subjectID in f:
                            ...
                            for side in ["left", "right"]:
                                ...
                                scan_list = glob.glob(
                                    root_dir + "/" + group + "/" + stage + "/" + subjectID + "*blmptrim_" + side + "_to_hw.npy")
...
def get_file_list_inter_subject(root_dir, groups, csv_out, stages):
...
def load_volfile(
    ...
    if 'crop' in augment:
        ...
        image_list1 = data_aug_cpu.randomCrop3d(image_list1, output_size)
...
def load_bl_fu_file(
    ...
    return [(vol_bl, affine_bl), (vol_fu, affine_fu)] if ret_affine else [vol_bl, vol_fu]
